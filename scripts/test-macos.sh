...
step "Validating M7 public API contracts"
...
cargo test -p luna-theme -p luna-qualification -p luna-ui-rust-qualification
...
cargo run --release -p luna-ui-rust-qualification -- --output "${TMPDIR:-/tmp}/luna-m7-macos-qualification.json"
...
./scripts/package-macos.sh --skip-build --output "${TMPDIR:-/tmp}/luna-m7-macos-package"

checklist="${TMPDIR:-/tmp}/luna-macos-runtime-checklist.txt"
cat > "$checklist" <<'CHECKLIST'
Luna-UI-Rust macOS M7 manual acceptance
...
Follow docs/EDITOR_DEMO_COMMANDS.md on both backends. Check Retina scaling, resize/full-screen, all five themes, native document-edited indication,
