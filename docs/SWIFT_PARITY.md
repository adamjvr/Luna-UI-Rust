...
reusable editor UI              75–84% parity
file/workspace/pane integration 75–83% parity
total Swift functional surface  approximately 80–85%
```
...
native Linux and macOS dialog boundaries, a mutable recursive workspace runtime, persistent recent/workspace
...
| IME | Native enable/disable/pre-edit/commit delivery, visible transient composition, candidate caret geometry, transactional commit, and macOS acceptance protocol | Broader accepted product integration | Rust core implemented; real-hardware breadth still pending |
...
| Native dialogs | Product-neutral contract plus Zenity/KDialog Linux, AppleScript macOS, and deterministic scripted adapters | Host-owned cross-platform Open, Save As, dirty-close, and product integration | Rust covers official platforms; Swift broader in product integration |
| Projects/workspaces | Open Folder, recursive snapshots, mutations, Linux native and macOS FSEvents watching, polling fallback, incremental subtree reconciliation, restoration, and file activation | Broader platform-native watchers, deeper product integration, and broader operations | Rust close in core runtime; Swift ahead in breadth |
...
downstream composition -> luna-integration
...
missing Rust GPU proof-backend gap and expands built-in palette coverage. M5 closes the reusable syntax-theme, history, multiple-selection, IME composition, dynamic
command-state, and editable action-routing gaps. M6 closes the missing macOS lifecycle, state-path,
dialog, FSEvents, packaging-seam, and generic downstream-composition gaps. Swift remains ahead in
regex and language-aware services, richer docking and cross-window movement, mature product-level
macOS integration, deeper downstream applications, and substantially broader phase-specific product
regression coverage.
